...
; (function (factory) {
            ...
            if (atLinkReg.test(text)) {
                ...
                if (settings.emailLink) {
                    text = text.replace(emailLinkReg, function ($1, $2, $3, $4, $5) {
                        return (!$2 && $.inArray($5, "jpg|jpeg|png|gif|webp|ico|icon|pdf".split("|")) < 0) ? "<a href=\"mailto:" + $1 + "\"target=\"_blank\">" + $1 + "</a>" : $1;
                    });
                }
    ...
    //         var tagRegex = new RegExp("<\\s*\\/?\\s*" + escapedTag + "\\s*[^>]*>", "igm");

    //         // 3. 核心替换：将匹配到的标签头/尾转义为纯文本（仅转义<>，保留内容）
    ...
    //         var htmlTagRegex = /<\s*(\w+)\s*([\s\S]*?)\s*(?:\/)?>/ig;
    ...
    //             html = html.replace(htmlTagRegex, function ($0, $1, $2) {
    //                 // 判断是否为自闭合标签
    //                 var isSelfClosing = /\/\s*>$/.test($0);
    //                 // 移除所有属性，返回纯净标签头
    //                 return isSelfClosing ? "<" + $1 + "/>" : "<" + $1 + ">";
    //             });
    //         } else if (attrs === "on*") {
    //             // html = html.replace(htmlTagRegex, function ($1, $2, $3, $4, $5) {
    //             //     var el = $("<" + $2 + ">" + $4 + "</" + $5 + ">");
    ...
    //             html = html.replace(htmlTagRegex, function ($0, $1, $2) {
    //                 // $0: 完整匹配的标签头（如 <img src=x onerror="alert(1)">）
    //                 // $1: 标签名（如 img）
    //                 // $2: 所有属性字符串（如 src=x onerror="alert(1)"）

    //                 // 步骤1：解析属性字符串，检测是否包含 on 开头的事件属性
    //                 let hasOnAttr = false;
    //                 let attrRegex = /([\w-]+)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/g;
    //                 let attrMatch;

    //                 while ((attrMatch = attrRegex.exec($2)) !== null) {
    //                     let attrName = attrMatch[1].trim();
    //                     // 判断是否为 on 开头的属性（不区分大小写）
    ...
    //                 // 步骤2：核心逻辑——有 on 开头属性则转义为纯文本，否则保留原标签头
    //                 if (hasOnAttr) {
    //                     // 转义 < 和 > 为 HTML 实体，让标签头变成纯文本
    ...
    //             // });
    //             // 步骤1：处理过滤列表——去空格、去空值、转小写（兼容大小写匹配）
    //             let filterAttrs = attrs.split(",")
    //                 .map(item => item.trim().toLowerCase()) // 转小写，实现不区分大小写匹配
    //                 .filter(item => item); // 过滤空字符串（如配置",href,"时的空值）;

    //             html = html.replace(htmlTagRegex, function ($0, $1, $2) {
    //                 // $0: 完整标签头（如 <a href="javascript:alert(1)">）
    //                 // $1: 标签名（如 a）
    //                 // $2: 所有属性字符串（如 href="javascript:alert(1)"）


    //                 // 步骤2：核心简化逻辑——直接检测 $2 中是否包含过滤列表的任意字符
    //                 var hasFilterChar = false;
    //                 var attrStrLower = $2.toLowerCase(); // $2 转小写，统一匹配规则
    //                 for (var i = 0; i < filterAttrs.length; i++) {
    //                     var filterChar = filterAttrs[i];
    //                     if (attrStrLower.includes(filterChar)) { // 直接检测字符串包含关系
    //                         hasFilterChar = true;
    //                         break; // 找到一个就停止，提升性能
    //                     }
    //                 }

    //                 // 步骤3：含过滤字符则转义为纯文本，否则保留原标签头
    //                 if (hasFilterChar) {
    //                     return $0.replace(/</g, '&lt;').replace(/>/g, '&gt;');
    //                 } else {
    //                     return $0;
    ...
    // filters = "style,script,iframe,sub,sup|on*,href|javascript,other"

    /**
     * 完整地过滤指定的HTML标签、属性名、属性值
     * Filter custom html tags
     * 使用的过滤规则 htmlDecode: "style,script,iframe,object,embed|on*|javascript,base64,data:",
     * @param   {String}   html          要过滤HTML
     * @param   {String}   filters       要过滤的标签
     * @returns {String}   html          返回过滤的HTML
     */
    editormd.filterHTMLTags = function (html, filters) {
        ...
        function checkAttrFilter(attrStr) {
            ...
            while ((attrMatch = attrRegex.exec(attrStr)) !== null) {
                ...
                // html实体解析
                attrValue = decodeHTMLEntities(attrValue);
